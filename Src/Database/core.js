        ...
        let promise = await new Promise(function (res, rej) {
            mongo.connect(MongoDB_URI, { useUnifiedTopology: true }, (err, db) => {
                if (err) {
                    ...
                    } else if (err == "MongoServerSelectionError: F8600000:error:0A000438:SSL routines:ssl3_read_bytes:tlsv1 alert internal error:c:\ws\deps\openssl\openssl\ssl\record\rec_layer_s3.c:1590:SSL alert number 80") {
    ...
    _Post = async function (collection, data) {
        ...
        return await this.#__Open()
            ...
            .then(async (collection) => {
                try{ data.createdAt = Date.now(); } catch(err){
